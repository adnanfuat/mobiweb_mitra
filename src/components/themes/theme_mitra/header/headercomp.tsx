...
import {Menu} from "@/themes/theme_mitra/menu/menu";
    ...
    return (
            <div className={s.headercompwr} style={{position, padding:`${topbottom}px ${sidepadding}px`}}>
                {/* {JSON.stringify(filename)} */}
                        <Link href={`/`}>
                          <div className={s.logowr} style={{ backgroundImage: `url(${`${process.env.NEXT_PUBLIC_IMGSOURCE}/${filename}`})`, backgroundSize:"contain" , backgroundPosition: 'center', backgroundRepeat:"no-repeat",  minWidth:200, minHeight:100}}>
                            {/* {`${process.env.NEXT_PUBLIC_IMGSOURCE}/${filename}`} */}
                          </div>
                        </Link>
