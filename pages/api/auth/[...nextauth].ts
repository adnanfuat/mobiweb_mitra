...
export const authOptions: NextAuthOptions = 
  {
    ...
    callbacks:{
      ...
      async session({ session, token,user }) {
                            ...
                            let contactmessages=await prisma.contents.findMany({select:{title_tr:true, slug_tr:true, bigdata:true}, where:{AND:[{type:"message"}, {bigparent_slug:"web"}, {parent_slug:"contact"}, {parent_key:"mitraemlak.com"}]}}); //devamlı bu verileri çekmesin diye cacheleme yapılabilir.. ya da backendde cachleme yapılabilir..                
